...
import { GeneralTaxRow, deleteGeneralTax, getGeneralTaxes, setFormToShow, setSelectedGeneralTax } from './generalTaxSlice';
...
const GeneralTaxesList: React.FC = () => {  
    const [open, setOpen] = React.useState(false);
    const handleOpen = () => setOpen(true);
    const handleClose = () => setOpen(false);

    const dispatch = useDispatch<AppDispatch>();
    const generalTaxes = useSelector((state: RootState) => state.generalTaxes.generalTaxes);  
    const client = useSelector((state: RootState) => state.clients.selectedClient);  

    const totalPages = useSelector((state: RootState) => state.generalTaxes.metaData?.totalPages);
    const currentPage = useSelector((state: RootState) => state.generalTaxes.metaData?.currentPage);
  
    const handleChange = (event: React.ChangeEvent<unknown>, value: number) => {
      dispatch(getGeneralTaxes({ PageSize: 10, PageNumber: value , clientId: client?.id || 0}));
    };
  

    const handleShow = (generalTax: GeneralTaxRow) => {
      dispatch(setSelectedGeneralTax(generalTax))
      dispatch(setFormToShow(3))
    }
  
    useEffect(() => {
      dispatch(getGeneralTaxes({ PageSize: 10, PageNumber: 1,  clientId: client?.id || 0}));
    }, [dispatch]);
  
    const [generalTaxDlete, setGeneralTaxDelete] = React.useState<GeneralTaxRow>()
    ...
    const handleDelete = (id: number) => {
      dispatch(deleteGeneralTax({id: id, clientId: client?.id || 0}))
      handleClose()
    }
            ...
            >
              <Box sx={style}>
                ...
                <div className="btns flex gap-3 mt-8 justify-center">
                    ...
                    <button onClick={() => handleDelete(generalTaxDlete.id)} className='inline-flex items-center justify-center gap-2.5 rounded-full py-4 px-10 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-10' style={{background: "#fb5454"}}>
                      حذف
                    </button>
                </div>
              </Box>
        ...
        }
        {
            <>
              <div className=" flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <nav>
                    <button className="inline-flex bg-primary items-center justify-center gap-2.5 rounded-full bg-meta-3 py-2 px-5 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-5" onClick={() => dispatch(setFormToShow(2))}>اضافة ضريبة عامة</button>
                </nav>
              </div>
              ...
              <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
                <div className="max-w-full overflow-x-auto">
                  <table className="w-full table-auto">
                    ...
                    <tbody>
                      {generalTaxes.map((generalTax:GeneralTaxRow) => (
                          <tr key={generalTax.id}>
                              ...
                              <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                <button onClick={() => handleShow(generalTax)} className="inline-flex bg-blck items-center justify-center gap-2.5 rounded-full bg-meta-3 py-2 px-4 text-center font-medium text-white hover:bg-opacity-90 lg:px-4 xl:px-4">
                      ...
                      {
                        !generalTaxes || generalTaxes.length === 0 && (
                          <tr>
                            <td colSpan={6}>
                              <h2 className='p-4 mt-4 text-center'>لا يوجد ضرائب حتى الان!</h2>
                            </td>
                          </tr>
                        )
                      }
...
export default GeneralTaxesList;
