...
import { BranchRow, deleteBranch, getBranches, setFormToShow, setSelectedBranch } from './branchSlice';
...
const BranchesList: React.FC = () => {  
    ...
    const totalPages = useSelector((state: RootState) => state.branches.metaData?.totalPages);
    const currentPage = useSelector((state: RootState) => state.branches.metaData?.currentPage);
  
    const handleChange = (event: React.ChangeEvent<unknown>, value: number) => {
      dispatch(getBranches({ PageSize: 10, PageNumber: value , clientId: client?.id || 0}));
    };

    const handleShow = (branch: BranchRow) => {
      dispatch(setSelectedBranch(branch))
      dispatch(setFormToShow(3))
    }
  
    useEffect(() => {
      dispatch(getBranches({ PageSize: 10, PageNumber: 1,  clientId: client?.id || 0}));
    }, [dispatch]);
    ...
    const handleDelete = (id: number) => {
      dispatch(deleteBranch({id: id, clientId: client?.id || 0}))
      handleClose()
    }
        ...
        }
        {
            <>
              <div className=" flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <nav>
                    <button className="inline-flex bg-primary items-center justify-center gap-2.5 rounded-full bg-meta-3 py-2 px-5 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-5" onClick={() => dispatch(setFormToShow(2))}>اضافة فرع جديد</button>
                </nav>
              </div>
                    ...
                    <tbody>
                      {branchs.map((branch:BranchRow) => (
                          <tr key={branch.id}>
                              ...
                              <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                <button onClick={() => handleShow(branch)} className="inline-flex bg-blck items-center justify-center gap-2.5 rounded-full bg-meta-3 py-2 px-4 text-center font-medium text-white hover:bg-opacity-90 lg:px-4 xl:px-4">
                      ...
                      ))}
                      {
                        !branchs || branchs.length === 0 && (
                          <tr>
                            <td colSpan={6}>
                              <h2 className='p-4 mt-4 text-center'>لا توجد فروع حتى الان!</h2>
                            </td>
                          </tr>
                        )
                      }
