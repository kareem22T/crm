...
import { ContractRow, deleteContract, getContracts, setFormToShow, setSelectedContract } from './contractSlice';
...
const ContractsList: React.FC = () => {  
    ...
    const totalPages = useSelector((state: RootState) => state.contracts.metaData?.totalPages);
    const currentPage = useSelector((state: RootState) => state.contracts.metaData?.currentPage);
  
    const handleChange = (event: React.ChangeEvent<unknown>, value: number) => {
      dispatch(getContracts({ PageSize: 10, PageNumber: value , clientId: client?.id || 0}));
    };
  


    const handleShow = (contract: ContractRow) => {
      dispatch(setSelectedContract(contract))
      dispatch(setFormToShow(3))
    }
  
    useEffect(() => {
      dispatch(getContracts({ PageSize: 10, PageNumber: 1,  clientId: client?.id || 0}));
    }, [dispatch]);
  
    const [contractDlete, setContractDelete] = React.useState<ContractRow>()
    ...
    const handleDelete = (id: number) => {
      dispatch(deleteContract({id: id, clientId: client?.id || 0}))
      handleClose()
    }
            ...
            >
              <Box sx={style}>
                ...
                <div className="btns flex gap-3 mt-8 justify-center">
                    ...
                    <button onClick={() => handleDelete(contractDlete.id)} className='inline-flex items-center justify-center gap-2.5 rounded-full py-4 px-10 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-10' style={{background: "#fb5454"}}>
                      حذف
                    </button>
                </div>
              </Box>
        ...
        }
        {
            <>
              <div className=" flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <nav>
                    <button className="inline-flex bg-primary items-center justify-center gap-2.5 rounded-full bg-meta-3 py-2 px-5 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-5" onClick={() => dispatch(setFormToShow(2))}>اضافة عقد جديد</button>
                </nav>
              </div>
                    ...
                    <tbody>
                      {contracts.map((contract:ContractRow) => (
                          <tr key={contract.id}>
                              ...
                              <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                <button onClick={() => handleShow(contract)} className="inline-flex bg-blck items-center justify-center gap-2.5 rounded-full bg-meta-3 py-2 px-4 text-center font-medium text-white hover:bg-opacity-90 lg:px-4 xl:px-4">
                      ...
                      ))}
                      {
                        !contracts || contracts.length === 0 && (
                          <tr>
                            <td colSpan={6}>
                              <h2 className='p-4 mt-4 text-center'>لا يوجد عقود حتى الان!</h2>
                            </td>
                          </tr>
                        )
                      }
